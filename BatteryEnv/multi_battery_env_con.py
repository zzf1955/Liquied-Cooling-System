...
class MutiBatteryEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.battery_system.batteries[0].current == self.current_mu:
            ...
            flow_rate  = 0
            inlet_temp = self.environment_temp
    ...
    def _calculate_reward(self, state, action, last_action):
        ...
        d_temp_penalty = (inlet_temp - last_action[0])/(self.inlet_temp_range[1]-self.inlet_temp_range[0])
        d_flow_penalty = (flow_rate - last_action[1])/(self.flow_rate_range[1]-self.flow_rate_range[0])
