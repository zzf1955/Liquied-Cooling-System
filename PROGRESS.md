...
## 2025-02-26: DSAC 多电池 RL 训练实验

### 背景
首次使用 DSAC (Deep Soft Actor-Critic) 算法在多电池液冷环境中进行训练实验。

### 训练配置
- **环境**: 4组 × 6电池
- **算法**: DSAC (Diffusion Soft Actor-Critic)
- **并行环境**: DummyVectorEnv (4个训练环境)
- **Epochs**: 500
- **每epoch步数**: 200步 (每步 = 5秒物理时间)
- **Batch size**: 256

### 奖励函数组成
1. **温度偏差惩罚**: 0.5 × (核心温度 - 目标温度)²
2. **温度达标奖励**: 温度在 ±5K 目标范围内时给予正奖励 (最高10分)
3. **组间温差惩罚**: 1.5 × (最高组温 - 最低组温)
4. **控制成本**: 0.5 × 流速 + 0.5 × |Δ入口温度|
...
**最佳 reward**: 6539.30 (epoch 477)

### 温度控制效果

| 指标 | 早期 (epoch 100) | 晚期 (epoch 900) |
|------|-----------------|-----------------|
| 平均温度 | 305.6K | **300.8K** |
| 温度范围 | 298-310K | 295-305K |
| 达标率 (±5K) | 10.0% | **53.4%** |
...
#### 问题1: 训练 reward 波动大，未稳定上升
- **现象**: 前期 reward 振荡剧烈，从 -1140 到 +5116 又掉到 -12996
- **原因分析**: step_per_epoch=200 太小，只相当于 1 个完整 episode
- **解决方案**: 保持现有配置，训练后期逐渐收敛

#### 问题2: SubprocVectorEnv 多进程同步问题
- **现象**: 多进程环境下出现 ConnectionResetError
- **原因**: 环境未正确关闭
- **解决方案**: 使用 DummyVectorEnv 替代，更稳定

#### 问题3: Tianshou API 使用错误
- **现象**: SACPolicy 初始化报错，offpolicy_trainer 缺少参数
- **解决方案**: 使用正确的参数位置和必需参数 (step_per_collect=None)
